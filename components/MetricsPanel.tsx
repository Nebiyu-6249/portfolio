  ...
  return (
    <div className="py-3.5">
      <div className="flex items-baseline justify-between gap-4">
        ...
        <span className="font-mono text-lg font-semibold tracking-tight text-[#E8B24E]">
      ...
      {hasBar ? (
        <div className="mt-2.5 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
          <motion.div
            className="h-full rounded-full bg-[#34C2AC]"
...
function BaselineCompare() {
  ...
  const rows = [
    { ...fraudBaseline.model, color: "#34C2AC", strong: true },
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="rounded-full bg-[#34C2AC]/15 px-2.5 py-1 font-mono text-[0.65rem] font-medium text-[#5fd7c3]">
            ...
            <div className="mb-1.5 flex items-baseline justify-between gap-3">
              ...
              <span
                className={`font-mono text-sm ${
                  r.strong ? "font-semibold text-[#E8B24E]" : "text-white/50"
                }`}
              >
                {r.value}
              </span>
...
export function MetricsPanel({
  ...
  return (
    <div className="overflow-hidden rounded-card border border-white/10 bg-gradient-to-b from-[#0C1A18] to-[#080F0D] text-white shadow-[0_30px_80px_-40px_rgba(0,0,0,0.6)]">
      <div className="border-b border-white/10 p-6 sm:p-7">
        <div className="flex items-center gap-2.5">
          <span className="relative flex h-2 w-2">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#34C2AC] opacity-60" />
            <span className="relative inline-flex h-2 w-2 rounded-full bg-[#34C2AC]" />
          </span>
          ...
          {groups.map((group) => (
            <div key={group.title}>
              <div className="mb-1 flex items-baseline justify-between border-b border-white/10 pb-2">
                <h4 className="font-mono text-xs font-medium uppercase tracking-wider text-[#5fd7c3]">
                  {group.title}
                </h4>
              </div>
              {group.caption ? (
